			...
			<table width="99%" style="margin:0px auto 0px auto;">
				<tr>
					<td>
						...
						<form id="editForm" action="brand.html?do=preEdit" method="post">
...
<script type="text/javascript">
	jQuery().ready(function (){
		jQuery("#list").jqGrid({
			url:"searchBrand.html",
			datatype: "json",
			height: "100%",
			autowidth: true,
			colNames:['<fmt:message key="brandID" />','<fmt:message key="name" />'],
			colModel:[
				{name:'brandID',index:'brandID', sorttype:"int"},
		...
		.navButtonAdd('#list_toppager',
		{
			...
			onClickButton: function(){ 
				window.location = '<c:url value="/brand.html?do=preAdd" />';
			}, 
			position:"last"
		})
		.navButtonAdd('#list_toppager',
	...
	function gridReload(){
		...
		jQuery("#list").jqGrid('setGridParam',{url:"searchBrand.html?name="+name,page:1}).trigger("reloadGrid");
	}

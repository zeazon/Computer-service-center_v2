			...
			<table width="99%" style="margin:0px auto 0px auto;">
				<tr>
					<td>
						...
						<form id="editForm" action="transportCompany.html?do=preEdit" method="post">
...
<script type="text/javascript">
	jQuery().ready(function (){
		jQuery("#list").jqGrid({
			...
			height: "100%",
			autowidth: true,
			colNames:['<fmt:message key="transportCompanyID" />','<fmt:message key="name" />'],
			colModel:[
				{name:'tcID',index:'tcID', sorttype:"int"},
		...
		.navButtonAdd('#list_toppager',
		{
			...
			onClickButton: function(){ 
				window.location = '<c:url value="/transportCompany.html?do=preAdd" />';
			}, 
			position:"last"
		})
		.navButtonAdd('#list_toppager',
			...
			onClickButton: function(){
				...
				if( gr != null ) {
					jQuery("#list").delGridRow(gr,{
						...
						url: 'transportCompany.html?do=delete&tcID='+gr}); 
				} else { 
					//alert("<fmt:message key='msg.pleaseSelectRow' />");
					jQuery("#dialog").text('<fmt:message key='msg.pleaseSelectRow' />');
					jQuery("#dialog").dialog( 
	...
	function gridReload(){
		...
		jQuery("#list").jqGrid('setGridParam',{url:"searchTc.html?name="+name,page:1}).trigger("reloadGrid");
	}

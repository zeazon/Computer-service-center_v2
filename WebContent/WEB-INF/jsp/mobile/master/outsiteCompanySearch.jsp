			...
			<table width="99%" style="margin:0px auto 0px auto;">
				<tr>
					<td>
						...
						<form id="editForm" action="outsiteCompany.html?do=preEdit" method="post">
...
<script type="text/javascript">
	jQuery().ready(function (){
		jQuery("#list").jqGrid({
			url:"searchOutsiteCompany.html",
			datatype: "json",
			height: "100%",
			autowidth: true,
			colNames:['<fmt:message key="outsiteCompanyID" />','<fmt:message key="name" />'],
			colModel:[
				{name:'ocID',index:'ocID', sorttype:"int"},
		...
		.navButtonAdd('#list_toppager',
		{
			...
			onClickButton: function(){ 
				window.location = '<c:url value="/outsiteCompany.html?do=preAdd" />';
			}, 
			position:"last"
		})
		.navButtonAdd('#list_toppager',
	...
	function gridReload(){
		...
		jQuery("#list").jqGrid('setGridParam',{url:"searchOutsiteCompany.html?name="+name,page:1}).trigger("reloadGrid");
	}

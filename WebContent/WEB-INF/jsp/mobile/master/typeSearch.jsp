			...
			<table width="99%" style="margin:0px auto 0px auto;">
				<tr>
					<td>
						...
						<form id="editForm" action="type.html?do=preEdit" method="post">
...
<script type="text/javascript">
	jQuery().ready(function (){
		jQuery("#list").jqGrid({
			...
			height: "100%",
			autowidth: true,
			colNames:['<fmt:message key="typeID" />','<fmt:message key="name" />'],
			colModel:[
				{name:'typeID',index:'typeID', sorttype:"int"},
		...
		.navButtonAdd('#list_toppager',
		{
			...
			onClickButton: function(){ 
				window.location = '<c:url value="/type.html?do=preAdd" />';
			}, 
			position:"last"
		})
		.navButtonAdd('#list_toppager',
			...
			onClickButton: function(){
				...
				if( gr != null ) {
					jQuery("#list").delGridRow(gr,{
						...
						url: 'type.html?do=delete&typeID='+gr}); 
				} else { 
					//alert("<fmt:message key='msg.pleaseSelectRow' />");
					jQuery("#dialog").text('<fmt:message key='msg.pleaseSelectRow' />');
					jQuery("#dialog").dialog( 
	...
	function gridReload(){
		...
		jQuery("#list").jqGrid('setGridParam',{url:"searchType.html?name="+name,page:1}).trigger("reloadGrid");
	}

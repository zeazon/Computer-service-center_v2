...
public class ServiceOrderServiceImpl implements ServiceOrderService {
	...
	public Map<String, Object> selectByCriteria2(String serviceOrderID, String name, String mobileTel, String startDate,
			String endDate, String type, String serialNo, String employee, String issuePartCode, Integer rows, Integer page, String orderBy,
			String orderType) {
		if(null != name && !name.equals("")) {
			name = "%"+name+"%";
		}
		...
		if(null != issuePartCode && !issuePartCode.equals("")) {
			issuePartCode = "%"+issuePartCode.trim()+"%";
		}
		Map<String, Object> result = new HashMap<String, Object>();
		try {
			result = soDAO.selectByCriteria2(serviceOrderID, name, mobileTel, startDate, endDate, type, serialNo, employee, issuePartCode, rows, page, orderBy, orderType);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	...
	public List<ServiceOrder> selectSOForCloseByCriteriaIssuePart(String name,
		...
		List<ServiceOrder> modelList = new ArrayList<ServiceOrder>();
		try {
			modelList = soDAO.selectSOForCloseByCriteriaIssuePart(name, startDate, endDate, type, serialNo, empFixID, issuePartCode, rows, page, orderBy, orderType);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return modelList;
	...
	public List<ServiceOrder> selectFixedSOByCriteria(String name, String date,
		...
		List<ServiceOrder> modelList = new ArrayList<ServiceOrder>();
		try {
			modelList = soDAO.selectFixedSOByCriteria(name, date, type, issuePartCode, rows, page, orderBy, orderType);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return modelList;
